...
class ListingCreate extends Component {
  constructor(props) {
    ...
    if (sessionStorage.getItem("createlisting")) {
      let storage_createlisting = JSON.parse(
        sessionStorage.getItem("createlisting")
      );
      this.state = {
  ...
  handleChange(e) {
    ...
    } else if (e.target.name == "client_number") {
      let client_number = e.target.value;
      if (!Number(client_number)) {
        return;
      }
      this.setState({
        [e.target.name]: client_number,
      });
    } else {
      this.setState({
  ...
  setAddress(value) {
    fetch(
      "https://maps.googleapis.com/maps/api/geocode/json?address=" +
        value.label +
        "&key=AIzaSyDkaV_9E9-b0FjMwak5UFwI0T1JtMrd_to"
    )
      .then((res) => res.json())
      .then(
        (result) => {
          ...
          this.setState({
            center: {
              lat: Math.floor(result.results[0].geometry.location.lat).toFixed(
                2
              ),
              lng: Math.floor(result.results[0].geometry.location.lng).toFixed(
                2
              ),
            },
          });
  ...
  handleSubmit(e) {
    ...
    if ($$("#date-picker-inline-helper-text").html() != undefined) {
      ...
      this.state.offer_amount < 15 &&
      this.state.switch_is_hidden == true
    ) {
      ToastsStore.error("Amount cannot be less than 15");
    } else if (
      this.state.offer_amount < 15 &&
      this.state.switch_is_hidden == false &&
      this.state.switch_value == true
    ) {
      ToastsStore.error("Amount cannot be less than 15");
    } else {
  ...
  handleMls(type, indexx) {
    ...
    if (type == "select") {
      ...
      rows[indexx] = {
        mls_id: mlsid,
        mls_text: mlstext,
        mls_link: rows[indexx].mls_link,
      };
    } else {
      let mlslink = $$(`#linkmls_${indexx}`).val();
      rows[indexx] = {
        mls_id: rows[indexx].mls_id,
        mls_text: rows[indexx].mls_text,
        mls_link: mlslink,
      };
    }
    this.setState({
                  ...
                  <div className="frm-wrapper text-left frm-wrapper-profile">
                    <form onSubmit={this.handleSubmit}>
                      ...
                      >
                        ...
                        <option value="Contractor Assistance">
                          Contractor Assistance
                        </option>
                        <option value="Appraisal/Inspection">
                      ...
                      <input
                        ...
                        name="keyword"
                        id="keyword"
                        placeholder="e.g. Bristlecone Drive"
                        ...
                        return (
                          <div className="row" key={indexx}>
                            <div className="col-lg-6">
                              <select
                                name="mlsid"
                                id={`selectmls_${indexx}`}
                                className="custom-select2"
                                onChange={() =>
                                  this.handleMls("select", indexx)
                                }
                                value={mlssingle.mls_id}
                              >
                                <option value="">Select</option>
                            ...
                            <div className="col-lg-6">
                              <input
                                ...
                                onChange={() => this.handleMls("link", indexx)}
                                value={mlssingle.mls_link}
                              />
                            </div>
                          </div>
                        );
                      ...
                      <div className="add-mls">
                        <a onClick={this.addRow}>MLS +</a>
                      </div>
                      <label>Date</label>
                      <Datepicker
                      ...
                      {this.state.switch_is_hidden == false ? (
                        ...
                        >
                          <CustomizedSwitches
                            ...
                            switch_value={this.state.switch_value}
                          />
                        </div>
                      ) : null}
                      ...
                      <input
                        ...
                        onChange={this.handleChange}
                        readOnly={this.state.readonly_offeramount}
                      />
...
export default withRouter(
  connect(mapStateToProps, mapDispatchToProps)(ListingCreate)
);
