export const SET_CURRENT_USER       = 'SET_CURRENT_USER';
export const NOTIFICATION_LIST      = 'NOTIFICATION_LIST';
export const MASTER_STATE           = 'MASTER_STATE';
export const MASTER_BROKERAGE       = 'MASTER_BROKERAGE';
export const MASTER_LICENSE         = 'MASTER_LICENSE';
export const CREATE_ACCOUNT         = 'CREATE_ACCOUNT';
export const DASHBOARD_LIST         = 'DASHBOARD_LIST';
export const PROFILE_DETAIL         = 'PROFILE_DETAIL';
export const PROFILE_PICTURE        = 'PROFILE_PICTURE';
export const ACTIVE_VIEW            = 'ACTIVE_VIEW';
export const PROFILE_UPDATE         = 'PROFILE_UPDATE';
export const LOGOUT_USER            = 'LOGOUT_USER';
export const FORGET_PASSWORD        = 'FORGET_PASSWORD';
export const FORGET_USERID          = 'FORGET_USERID';
export const RESET_PASSWORD         = 'RESET_PASSWORD';
export const LISTING_ACTIVE_VIEW    = 'LISTING_ACTIVE_VIEW';
export const MASTER_MLS             = 'MASTER_MLS';
export const MY_LISTING             = 'MY_LISTING';
export const SAVE_LISTING           = 'SAVE_LISTING';
export const REGISTRATION_VERIFICATION      = 'REGISTRATION_VERIFICATION';
export const RESEND_OTP                     = 'RESEND_OTP';
export const LISTING_STORAGE                = 'LISTING_STORAGE';
export const DETAIL_LISTING                 = 'DETAIL_LISTING';
export const CONTACT_US             = 'CONTACT_US';
export const STATUS_LISTING_CANCEL  = 'STATUS_LISTING_CANCEL';
export const NOPAY_LISTING_CANCEL   = 'NOPAY_LISTING_CANCEL';
export const WITHPAY_LISTING_CANCEL = 'WITHPAY_LISTING_CANCEL';
export const PAYMENT_METHOD_LIST    = 'PAYMENT_METHOD_LIST';
export const SEARCH_OFFER_LIST = "SEARCH_OFFER_LIST";
export const CLEAR_OFFER_LIST = "CLEAR_OFFER_LIST";
export const CLEAR_ACCEPT_LISTING_SUCCESS = "CLEAR_ACCEPT_LISTING_SUCCESS";
export const ACCEPT_LISTING_SUCCESS = "ACCEPT_LISTING_SUCCESS";
export const SET_MYCARD_LIST = "SET_MYCARD_LIST";
export const RETURN_LISTING_SUCCESS = "RETURN_LISTING_SUCCESS";
export const CLEAR_RETURN_LISTING_SUCCESS = "CLEAR_RETURN_LISTING_SUCCESS";
export const PAYMENT_REQUEST_SUCCESS = "PAYMENT_REQUEST_SUCCESS";
export const CLEAR_PAYMENT_REQUEST_SUCCESS = "CLEAR_PAYMENT_REQUEST_SUCCESS";
export const SET_COMPLETE_CARD_DETAILS = "SET_COMPLETE_CARD_DETAILS";
export const DWOLLA_FUND_TRANSFER_SUCCESS = "DWOLLA_FUND_TRANSFER_SUCCESS";
export const CLEAR_DWOLLA_FUND_TRANSFER_SUCCESS = "CLEAR_DWOLLA_FUND_TRANSFER_SUCCESS";
